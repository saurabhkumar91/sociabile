...
{ 
    ...
    public function postCommentsAction($header_data,$post_data) {
        if(!isset($post_data['post_id']) || !isset($post_data['comment'])) {
            ...
            try {
                ...
                Library::logging('error',"API : postComments : ".$e->getMessage()." ".": user_id : ".$header_data['user_id']);
    ...
    public function getCommentsAction($header_data,$post_id) {
         if(empty($post_id)) {
            Library::logging('alert',"API : getComments : ".ERROR_INPUT.": user_id : ".$header_data['user_id']);
            Library::output(false, '0', ERROR_INPUT, null);
        } else {
            try {
                ...
                db.comments.find({"post_id":"'.$post_id.'","user_id":"'.$header_data['user_id'].'"}).forEach(
                ...
                Library::logging('error',"API : getComments : ".$e." ".": user_id : ".$header_data['user_id']);
                Library::output(false, '0', ERROR_REQUEST, null);
            }
        }
    }
