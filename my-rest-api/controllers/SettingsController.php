    ...
    public function generateOTPAction($header_data,$post_data){ 
        try {
            if( !isset($post_data['type'])) {
                ...
                $otp    = Library::getOTP();
                if($user->mobile_no=="7503522085"||$user->mobile_no=="+17503522085"||$user->mobile_no=="+917503522085" || $user->mobile_no=="8503522085"||$user->mobile_no=="+18503522085"||$user->mobile_no=="+918503522085"){
                    $otp    =   "1614";
                }
...
//            Library::logging('error',"API : contactUsLogging, request: ".  var_export($post_data,true).": user_id : ".$header_data['id']);
            ...
            if( isset($post_data['reported_post_id']) ){
                $reportedPost   = Posts::findById( $post_data['reported_post_id'] );
                $reportedUser   = Users::findById( $reportedPost->user_id );
    ...
    public function setPrivacySettingsAction($header_data,$post_data){
        if( (!isset($post_data['group_ids']) && $header_data['os'] == 1) || !isset($post_data['type'])) {
            ...
            try {
                ...
                $blockedGroup   = Groups::find( array( array("group_name"=>"Blocked", "is_active"=>1) ) );

                if( !empty($blockedGroup[0]) ){
                    foreach($blockedGroup AS $group){
                        if( in_array( (string)$group->_id, $group_ids) ){
            ...
            try {
                ...
                foreach( $sharedWith as $friendId ){
                    ...
                    if( !empty($friend->os) && in_array($friend->os, array("1", "2")) && !empty($friend->device_token) ){
                        ...
                        $this->sendNotifications( array($friend->device_token), array("message"=>json_encode($message)), $sendTo );
                    }
                }
...
//                    curl_close($ch);
    ...
    function sendNotifications( $deviceToken, $message, $os, $createLog=true, $savetoDb=true ){
    ...
    function sendNotification($headerdata, $post_data){
        ...
        $notifications  =   new NotificationsController();
        $notifications->sendNotifications( $post_data["token"], array("message"=>$post_data["message"]), $post_data["os"] );
    }
